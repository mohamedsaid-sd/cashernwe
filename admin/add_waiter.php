...
if($name != "")
{
$aq = mysqli_query($conn , "INSERT INTO `waiter` (`id`, `name`, `phone`) VALUES (NULL, '$name', '$phone');");
if($aq){
...
if(isset($_POST['eproduct'])){
	...
	$aq = mysqli_query($conn , "UPDATE `waiter` SET `name` = '$name', `phone` = '$phone' WHERE `waiter`.`id` = $id;");
	if($aq){
...
<body>
		...
		<table id="example" class="display" style="background-color: #fff;box-shadow: 2px 2px 5px black;border-radius: 0px;color: #000;" width="100%" >
			<tr>
				...
				<th style="font-size: 20px;text-align: center;"> الهاتف </th>
	...
	<center id="sectionRight">
		...
		{
		$id = $_GET['edid'] ; 
		$q = mysqli_query($conn,"SELECT * FROM `waiter` where id = $id ");
		while ($row = mysqli_fetch_array($q)) {
		 ?>
		<form action="add_waiter.php" method="post">
		...
		<input type="text" placeholder="إسم المستخدم" name="phone" value="<?php echo $row['phone']; ?>" /><br/>
